...
order. `n_R^act` and `n_S^act` are each side's **current** commitment number at the
...
`R` with a fresh random ephemeral key and the final payload
`{amt_to_forward = d_k, outgoing_cltv_value = T_exp, payment_data = {payment_secret =
SHA256("ffor/voucher-secret" ‖ epoch_id ‖ [2: k]), total_msat = d_k}}`. It exists so that
...
§7.5. Appendix D (transcript and both-view commitment vectors for §9.5.1) is
specified by M8.1 and is TBD at this revision.
