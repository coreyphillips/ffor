...
- **In Variant D the `H_1` binding is forbidden, not merely unused.** Variant D's setup
  ...
  `H_1` equals `SHA256` of any secret `S` has revealed. Nothing is lost: Variant D
...
route hint `S → R`, `min_final_cltv_expiry` as usual (it binds `S`'s upstream
...
**Amount, by variant.** In Variants A and B invoices are **amountless** (the payer
supplies the amount, and `R` cannot pre-sign an unknown one). In **Variant D** they are
**fixed-amount** and MUST be: the voucher for `H_k` is a committed HTLC of a known value
`d_k`, so `R` signs for `d_k + fee(d_k)` and `S` accepts the payment only at that amount
(§9.5.1 step 3). This is also what repairs amount attestation for Variant D (§13.3).
...
- **Both** parties' post-update balances ≥ their respective `channel_reserve`, with the
  per-voucher commitment weight fee charged to whichever side is the funder. When `R` is
  the funder this is not a formality: each voucher adds 172 WU of commitment fee to
  `R`'s side, so `R`'s balance falls as its credit rises, and `R` is offline and cannot
  object. `S` MUST run the check against the funder identity recorded at setup, not
  against itself.
