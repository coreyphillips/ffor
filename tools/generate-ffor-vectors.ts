 ...
 *     ../ffor/tools/generate-ffor-vectors.ts > ../ffor/ffor-test-vectors.md
 *
...
w();
w('FFOR 7.6, fixed-amount A/B profile. These vectors use the book fee;');
w('the public plaintext Variant D exception in 7.6 does not apply here.');
w('The inputs are the payee amounts `d_k`');
...
w();
w('```sh');
w('cd <beignet repo>   # sibling of ffor; use the revision in IMPLEMENTATION.md');
w('./node_modules/.bin/ts-node -P ../ffor/tools/tsconfig.json \\');
w('  ../ffor/tools/generate-ffor-vectors.ts > ../ffor/ffor-test-vectors.md');
w('```');
w();
...
w('entire file to stdout. See [IMPLEMENTATION.md](IMPLEMENTATION.md) for the pinned');
w('reference revision and dependency setup. Output is deterministic: running it twice yields');
...
w('---');
w();
w('*Generated by `tools/generate-ffor-vectors.ts` against beignet using');
